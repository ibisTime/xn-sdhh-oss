...
function _getMenuState({ data, pathname }) {
  ...
  if (pathname !== '/') {
    ...
    if (result.topMenuList[0]) {
      let code = result.topMenuList[0].code;
      let subMenu = getSubCode(code, result);
      result.subOpenCode = subMenu.subOpenCode;
      result.subMenuCode = subMenu.subMenuCode;
      result.subMenuList = subMenu.subMenuList;
      result.topMenuCode = code;
      let leftMenu = result.top2SubObj[code][0];
